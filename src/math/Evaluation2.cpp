...
int Evaluation2::getWeight(char op) {
    if ((op == '+') || (op == '-'))
        return 1;
    else if ((op == '*') || (op == '/'))
        return 2;
...
bool Evaluation2::hasPriority(char op1, char op2) {
    if (op2 == '(' || op2 == ')')
        return false;
    if ((op1 == '*' || op1 == '/') && (op2 == '+' || op2 == '-'))
        return false;
...
Result Evaluation2::executeToken(char op, bool isOperand, std::string &operand) {
//    if (isOperand){
//        ss += operand;
//    } else {
//        ss += op;
...
char *Evaluation2::getValue(bool isError, std::string &error, int *len) {
    if (isError)
